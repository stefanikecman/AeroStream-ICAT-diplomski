...
class DroneEnv(object):
    ...
    def get_obs(self):
        if self.useDepth:
            ...
            #uzima se samo prvi response
            image_array = Image.fromarray(image).resize((84, 84)).convert("L") #udaljenosti objekata koje vidim preko depth slike
            #koristit cemo subset info iz ovog image_array (umjesto svih udaljenosti, koristiti n najblizih tacaka n*1 vektor)
            #drugi nacin je da pored tih tacaka koristimo i kako su one pozicionirane u odnosu na dron (ugao pogleda - vertikalni i azimut ugao - n*3 vektor)
        else:
            # Get rgb image
            responses = self.client.simGetImages(
            ...
            image = img1d.reshape(response.height, response.width, 3) #ovo je rgb slika koju mi ne koristimo
    ...
    def compute_reward(self, quad_state, quad_vel, collision):
        """Compute reward"""
    #ovde treba izmijeniti reward tako da on koristi najmanju udaljenost od cilja
    #najmanja udaljenost je najmanji element matrice 84*84 image_array u get obs

    #reward je veći što je letjelica bliža cilju, a što je bliza nekom obstacle, onda je vrijednost manja
       #zbog treniranja sve funkcije trebaju biti diferencijalne i neprekidne
        reward = -1

...
class Agent:
    def __init__(self, useGPU=False, useDepth=False):
        ...
        obs, img = self.env.reset() #FALILO JE IMG, JER RESET VRAĆA 2 STVARI!!!!!!
        print ("Sada je obs: ", obs.shape)
        #drugi array u obs ima broj redova 144
        #print (obs[1].shape)

        #probat cu obako:
        #tensor = self.transformToTensor(obs[0]) #ovo je prva proba rjesenja, bez obs,img (samo obs)
        tensor = self.transformToTensor(obs) #ovo je original
    ...
    def act(self, state):
        ...
        if random.random() > self.eps_threshold:
            ...
            action = random.randrange(0, 4) #ovo je hardcodirano, trebat ce se mijenjati u buducnosti.
            return int(action)
